...
contract Campaignable {
    ...
    modifier isValidCreate(
        ...
        bytes32 messageHash = keccak256(abi.encode(_contractAddress, msg.sender, _campaignId, "create", _fee));
        address signer = messageHash.toEthSignedMessageHash().recover(_signature);
        require(signer == _authorizerAddress, "Invalid signature");
        _;
    }

    modifier isValidReserve(
        bytes memory _signature,
        uint256 _campaignId,
        address _contractAddress,
        address _authorizerAddress
    ) {
        bytes32 messageHash = keccak256(abi.encode(_contractAddress, msg.sender, _campaignId, "reserve"));
        address signer = messageHash.toEthSignedMessageHash().recover(_signature);
        require(signer == _authorizerAddress, "Invalid signature");
        _;
    }
